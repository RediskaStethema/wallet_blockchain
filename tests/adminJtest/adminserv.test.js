...
const service = new adminService(tronWebMock);
const { pool, Role, TransactionStatus } = modeles;
describe('adminService', () => {
    ...
    test('getAllWallets: returns wallets', async () => {
        ...
        pool.query.mockResolvedValue([wallets]);
    ...
    test('getWalletsByUser: returns wallets by user', async () => {
        ...
        expect(pool.query).toHaveBeenCalledWith('SELECT id, address, created_at FROM wallets WHERE user_id = ?', [1]);
        expect(result).toEqual(wallets);
    });
    ...
    test('getAllTransactions: returns transactions', async () => {
        ...
        pool.query.mockResolvedValue([txs]);
        const result = await service.getAllTransactions();
        expect(pool.query).toHaveBeenCalledWith(`SELECT t.id, t.tx_id, t.amount, t.status, t.created_at, w.address AS wallet
       FROM transactions t
    ...
    test('getTransactionsByUser: returns transactions by user', async () => {
        ...
        expect(pool.query).toHaveBeenCalledWith(`SELECT t.id, t.tx_id, t.amount, t.status, t.created_at 
       FROM transactions t
       JOIN wallets w ON t.wallet_id = w.id
       WHERE w.user_id = ?`, [1]);
        expect(result).toEqual(txs);
    ...
    test('registerAdmin: inserts new admin and returns token', async () => {
        ...
        expect(pool.query).toHaveBeenCalledWith('INSERT INTO users (email, password, role) VALUES (?, ?, ?)', ['a@b.com', 'hashed-password', Role.ADMIN]);
